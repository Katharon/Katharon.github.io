      ...
      <Container>
        ...
        <div className="mt-9 grid gap-5 md:grid-cols-2">
          {projects.map((project) => {
            const preview = getProjectPreview(project.name);

            return (
              <Card key={project.name} className="flex h-full flex-col overflow-hidden p-0">
                {preview ? (
                  <div className="relative aspect-[16/9] border-b border-white/10 bg-slate-950">
                    <Image
                      src={preview.src}
                      alt={preview.alt}
                      fill
